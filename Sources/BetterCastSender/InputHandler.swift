...
class InputHandler {
    ...
    func handle(event: InputEvent, for connectionId: UUID) {
        guard event.isValidForTransport, event.type != .command else { return }

        guard let bounds = displayBoundsMap[connectionId], bounds.width > 0, bounds.height > 0 else {
            LogManager.shared.log("InputHandler: ignoring input without verified display bounds")
            return
        }
